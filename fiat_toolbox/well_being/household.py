...
from .methods import recovery_time, recovery_rate, opt_lambda, equity_weight, wellbeing_loss, ReconstructionCost, IncomeLoss, ConsumptionLoss, UtilityLoss
...
class Household:
    ...
    def __init__(self, v: float, k_str: float, c0: float, c_avg: float, l: Optional[float] = None, 
                 pi: Optional[float] = 0.15, eta: Optional[float] = 1.5, rho: Optional[float] = 0.06, 
                 t_max: Optional[float] = 10, dt: Optional[float] = 1 / 52, currency: Optional[str] = "$") -> None:
        """
    ...
    def calc_loss(self, loss_type: LossType, method: str="trapezoid") -> float:
        ...
        if loss_type == LossType.RECONSTRUCTION:
            ...
            loss = UtilityLoss(self.t, self.l, self.v, self.k_str, self.pi, self.c0, self.eta)
        else:
    ...
    def get_losses(self, method: str="trapezoid") -> pd.Series:
        ...
        ut_t = UtilityLoss(t=self.t, l=self.l, v=self.v, k_str=self.k_str, pi=self.pi, c0=self.c0, eta=self.eta)
        ...
        ew_loss = equity_weight(c=self.c0, c_avg=self.c_avg, eta=self.eta) * self.v * self.k_str
    ...
    def plot_loss(self, loss_type: LossType, ax: Optional[plt.Axes] = None) -> Optional[plt.Figure]:
        ...
        if loss_type not in LossType.__members__.values():
            ...
            raise ValueError(f"Invalid type '{loss_type}'. Must be one of {valid_values}.")
        if ax is None:
            ax_given = False
            fig, ax = plt.subplots(figsize=(8, 6))
        sns.lineplot(x="time", y=loss_type, data=self.time_series, ax=ax)
        ax.fill_between(self.time_series.index, 0, self.time_series[loss_type], edgecolor='gray', alpha=0.3, label=f'Total {loss_type}: {self.total_losses[loss_type]:.2f} {self.currency}')
    ...
    def plot_consumption(self, ax: Optional[plt.Axes] = None) -> Optional[plt.Figure]:
        ...
        if LossType.CONSUMPTION not in self.time_series.columns:
            raise ValueError("Losses have not been calculated. Run the 'get_losses' method first.")
        ...
        ax.fill_between(self.time_series["time"], self.c0 - self.time_series[LossType.INCOME], self.c0, color=color1, alpha=0.6, label=label1)
        ...
        ax.fill_between(self.time_series["time"], self.c0 - self.time_series[LossType.CONSUMPTION], self.c0 - self.time_series[LossType.INCOME], color=color2, alpha=0.6, label=label2)
        ...
        expanded_consumption_losses = np.insert(self.c0 - self.time_series[LossType.CONSUMPTION], 0, [self.c0, self.c0])
        ax.plot(expanded_time, expanded_consumption_losses, linestyle='--', color='red', label='Consumption')
           
        # Plot recovery time
        ax.axvline(x=self.recovery_time, color='black', linestyle='-', alpha=0.3, label=f'Reconstruction rate: {self.l:.2f}')
        # Add text annotation for recovery time
        y_point = self.c0 - self.time_series[LossType.CONSUMPTION].mean()
        ax.text(self.recovery_time + 0.1, y_point, f'Recovery time: {self.recovery_time:.2f} years', 
            verticalalignment='center', horizontalalignment='left', color='black', fontsize=10, alpha=0.7)

        # Add lightning icon at t=0 years just above self.c0
        lightning_icon = u'\u26A1'
        ax.text(0, self.c0 + 0.1, lightning_icon, fontsize=50, color='red', verticalalignment='bottom', horizontalalignment='center')
    ...
    def opt_lambda(self, rec_time_max: Optional[float] = None, rec_time_min: float = 0.3, no_steps: float = 1000, method="trapezoid") -> None:
        ...
        reconstruction_costs = ReconstructionCost(t=self.t, l=lambdas, v=self.v, k_str=self.k_str).total(rho=0, method=method)
    ...
    def plot_opt_lambda(self, x_type: Literal["rate", "time"] = "rate") -> plt.Figure:
        ...
        if not hasattr(self, 'l_opt'):
            raise ValueError("Optimal lambda has not been calculated. Run the 'opt_lambda' method first.")
        ...
        sns.lineplot(x=x, y=self.l_opt[LossType.RECONSTRUCTION], color="green", ax=axs[0], label=LossType.RECONSTRUCTION)
        sns.lineplot(x=x, y=self.l_opt[LossType.INCOME], color="blue", ax=axs[0], label=LossType.INCOME)
        sns.lineplot(x=x, y=self.l_opt[LossType.CONSUMPTION], color="purple", ax=axs[0], label=LossType.CONSUMPTION)
        # Add vertical line for optimal lambda
        axs[0].axvline(x=val, color='red', linestyle='--', label=f'Optimum value: {val:.2f}')
        # Fill between the tested lambda values
        ylims = (axs[0].get_ylim()[0], axs[0].get_ylim()[1])
        axs[0].fill_between(x, ylims[0], ylims[1], color='steelblue', alpha=0.3, label='Tested range')
        ...
        sns.lineplot(x=x, y=self.l_opt[LossType.UTILITY], color="black", ax=axs[1], label=LossType.UTILITY)
        axs[1].axvline(x=val, color='red', linestyle='--', label=f'Optimum value: {val:.2f}')
        ylims = (axs[1].get_ylim()[0], axs[1].get_ylim()[1])
        axs[1].fill_between(x, ylims[0], ylims[1], color='steelblue', alpha=0.3, label='Tested range')
        axs[1].set_ylabel(f'Total Wellbeing Loss')
