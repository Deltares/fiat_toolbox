...
def test_consumption_loss_t():
    t = np.linspace(0, 1, 5)
    l, v, k_str, pi = 0.5, 0.2, 100000, 0.1
    cl = methods.consumption_loss_t(t, l, v, k_str, pi, savings=1000, insurance=500, support=200)
...
def test_consumption_t():
    ...
    ct = methods.consumption_t(t, l, v, k_str, pi, c0, cmin=1000, savings=1000, insurance=500, support=200)
    assert ct.shape == t.shape

def test_utility_loss_t():
    t = np.linspace(0, 1, 5)
    l, v, k_str, pi, c0, eta = 0.5, 0.2, 100000, 0.1, 15000, 1.5
    ul = methods.utility_loss_t(t, l, v, k_str, pi, c0, eta, cmin=1000, savings=1000, insurance=500, support=200)
